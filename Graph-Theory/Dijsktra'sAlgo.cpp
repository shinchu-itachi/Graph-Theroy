...
void solve() {
    // program to implement the dijskra's algorithm

    int n , m ; cin >> n >> m;
    dis.resize(n + 1 , 0);
    for (int i = 1 ; i <= n ; i ++) grp[i].clear() , dis[i] = mod1;
    ...
    while ( !pq.empty()) {
        ...
        for ( auto i : grp[node]) {
            ...
            int secnode = i.first;
            // cout << nextdis << ' ' << secnode << endl;
            if ( dis[secnode] > nextdis + disnode ) {
                dis[secnode] = nextdis + disnode;
                pq.push( make_pair(dis[secnode] , secnode));
            }
        }
